  ...
  const getStatusInfo = (status: string, type: 'tour' | 'companion', payPercent: number = 100) => {
    switch (status) {
      ...
      case 'cancelled_by_guide':
        return { label: 'HDV đã hủy tour', variant: 'danger' as const };
      case 'refund_pending':
        return { label: 'Chờ hoàn tiền', variant: 'warning' as const };
      case 'refunded':
                    ...
                    <div className="tc-card-actions">
                      {activity.type === 'tour' ? (
                        <>
                          ...
                          {(activity.status === 'pending' || activity.status === 'approved' || activity.status === 'paid') && (
                    ...
                    <div className="tc-transaction-table">
                      ...
                      {activity.transactions.map(tx => (
                        <div key={tx.id} className="tc-tr">
                          ...
                          <span>
                            <Badge variant={tx.status === 'paid' ? 'success' : tx.status === 'refunded' ? 'secondary' : tx.status === 'refund_pending' ? 'warning' : tx.status === 'cancelled' ? 'danger' : 'warning'}>
                              {tx.status === 'paid' ? 'Thành công' : tx.status === 'refunded' ? 'Đã hoàn tiền' : tx.status === 'refund_pending' ? 'Chờ hoàn tiền' : tx.status === 'cancelled' ? 'Đã hủy' : 'Đang chờ'}
                            </Badge>
                          </span>
                        </div>
                      ))}
