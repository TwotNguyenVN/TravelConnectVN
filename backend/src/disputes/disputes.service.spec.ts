...
describe('DisputesService', () => {
  ...
  describe('createDispute', () => {
    it('should throw NotFoundException if tour request is missing', async () => {
      ...
      await expect(service.createDispute('invalid-id', 'user-1', 'Reason')).rejects.toThrow(NotFoundException);
    });

    it('should open dispute and update request status to disputed', async () => {
      mockPrisma.tour_requests.findUnique.mockResolvedValue({ id: 'req-1', status: 'approved', tour_id: 'tour-1' });
      mockPrisma.tour_disputes.create.mockResolvedValue({ id: 'dispute-1', tour_request_id: 'req-1' });

      const result = await service.createDispute('req-1', 'user-1', 'Hủy tour không báo trước');
  ...
  describe('getDisputeChatHistory', () => {
    it('should throw NotFoundException if dispute does not exist', async () => {
      ...
      await expect(service.getDisputeChatHistory('invalid-id')).rejects.toThrow(NotFoundException);
    });

    it('should retrieve message list for matching conversation', async () => {
      mockPrisma.tour_disputes.findUnique.mockResolvedValue({
        id: 'dispute-1',
        tour_requests: { tour_id: 'tour-1', user_id: 'cust-1', tours: { guide_profile_id: 'guide-p-1' } },
      });
      mockPrisma.guide_profiles.findUnique.mockResolvedValue({ user_id: 'guide-1' });
      mockPrisma.conversations.findFirst.mockResolvedValue({ id: 'conv-1' });
      mockPrisma.messages.findMany.mockResolvedValue([{ id: 'msg-1', content: 'Halo' }]);
  ...
  describe('resolveDispute', () => {
    it('should resolve dispute, update request status and trigger refunds when applicable', async () => {
      mockPrisma.tour_disputes.findUnique.mockResolvedValue({ id: 'dispute-1', status: 'open', tour_request_id: 'req-1' });
      mockPrisma.tour_disputes.update.mockResolvedValue({ id: 'dispute-1', status: 'resolved' });
      mockPrisma.payment_transactions.findFirst.mockResolvedValue({ id: 'tx-1', status: 'paid' });
