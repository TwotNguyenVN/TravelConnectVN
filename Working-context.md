...
### 2026-06-12 (Backend Debug, Redis Stability, CORS & Tour Validation)
- **Sửa lỗi Backend bị Crash do Redis (EADDRINUSE & SocketClosedUnexpectedlyError):**
  - Tìm và giải phóng cổng 3000 bị chiếm dụng bởi các tiến trình nền cũ.
  - Cấu hình kết nối Upstash Redis trong `app.module.ts`, `redis-io.adapter.ts`, `socket/redis.adapter.ts` sử dụng giao thức `rediss://` (có TLS và mật khẩu).
  - Khắc phục lỗi ngắt kết nối không mong muốn (Uncaught exceptions từ Upstash Redis như `ECONNRESET`, `SocketClosedUnexpectedlyError`) bằng cách thêm trình lắng nghe sự kiện lỗi `pubClient.on('error', ...)` và `subClient.on('error', ...)` trong `redis.adapter.ts`. Điều này ngăn không cho lỗi socket làm sập máy chủ NestJS trong chế độ watch.
- **Cấu hình CORS để tránh bị chặn trình duyệt:**
  - Cập nhật danh sách origin cho phép trong `main.ts` của backend gồm `localhost:5173`, `127.0.0.1:5173`, `localhost:8080`, và `127.0.0.1:8080` để hỗ trợ chạy thử nghiệm trên môi trường local.
- **Sửa lỗi DTO Validation (HTTP 400 Bad Request) khi tạo Tour:**
  - Bổ sung đầy đủ các decorator validation từ `class-validator` (`@IsString()`, `@IsOptional()`, `@IsNumber()`, `@IsArray()`, v.v.) vào `CreateTourDto` và các class DTO liên quan trong `tours.dto.ts`. Điều này ngăn chặn bộ lọc whitelist của `ValidationPipe` loại bỏ các trường dữ liệu hợp lệ và trả về mã lỗi 400.
- **Khắc phục lỗi tham chiếu dữ liệu Tour:**
  - Khắc phục sự bất hợp lệ tham chiếu khi các tour trỏ đến hướng dẫn viên chưa hoàn thiện hồ sơ. Đã gán các tour này tới hướng dẫn viên hợp lệ (`Guider` - ID `b51fe0ac-e4d3-4fed-ac0f-a0f1bc7b7f2d`) để khôi phục việc hiển thị dữ liệu trên trang chủ.
- **Sửa lỗi CI/CD & Docker Build:**
  - Sửa lỗi CI: Sửa Prisma Schema `tour_images` (chuyển từ quan hệ 1-to-1 sang 1-to-many) và cập nhật TypeScript bindings trong `app.module`, `recommendations.service`, `tour-requests.service`.
  - Sửa lỗi Docker Build CI: Copy thư mục `prisma` và tệp cấu hình `prisma.config.ts` trước khi chạy `npm ci` trong `Dockerfile`.
  - Sửa lỗi Playwright CI: Loại bỏ `@default(dbgenerated(...))` bị lỗi của cột `email` trong bảng `auth.identities` và cột `confirmed_at` trong bảng `auth.users` ở `schema.prisma` để có thể chạy Migration ổn định trên môi trường kiểm thử.
