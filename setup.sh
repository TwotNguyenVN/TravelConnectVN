...
install_node() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        if ! command -v brew >/dev/null 2>&1; then
            echo "Chưa tìm thấy Homebrew. Vui lòng cài đặt Homebrew trước (https://brew.sh/) hoặc cài Node.js thủ công."
            exit 1
        fi
        ...
        echo "LƯU Ý QUAN TRỌNG TRÊN WINDOWS: Việc cài đặt qua winget sẽ yêu cầu bạn KHỞI ĐỘNG LẠI Git Bash để nhận diện lệnh 'node'."
