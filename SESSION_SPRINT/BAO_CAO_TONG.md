...
Dựa trên tài liệu sprint và source code, hệ thống được mô hình hóa bằng 6 sơ đồ UML (xem chi tiết tại `UML_DIAGRAMS.md`):

- **Use Case Diagram:** 29 chức năng, 4 actor chính (Guest, User, Guide, Admin). Suy ra từ `TRAVEL_PROJECT_MASTER_SPEC_v3_FINAL.md`.
- **ERD:** 42 bảng public schema với quan hệ rõ ràng. Bằng chứng: `database/ERD.txt`.
- **Activity Diagram — Đặt Tour & Thanh Toán:** Luồng từ xem tour → chọn ngày → thanh toán VNPAY → callback IPN.
- **Activity Diagram — Đăng ký & Phân quyền RBAC:** Luồng đăng ký → chọn vai trò → xác minh HDV.
- **Activity Diagram — Admin Xử lý Báo cáo:** Luồng gửi báo cáo → admin xem xét → resolve/dismiss.
- **Sequence Diagram — Chat Realtime:** Frontend → Socket.io Gateway → Backend → DB.
- **Sequence Diagram — Tìm Bạn Đồng Hành:** User tạo bài → User khác gửi request → duyệt → chat nhóm.
- **Component Diagram — Kiến trúc tổng thể:** Frontend ↔ Backend ↔ Database ↔ External services.
...
| Lưu trú | API + hiển thị trên TourDetailPage | accommodations module, TourDetailPage | Có API + UI card trên chi tiết tour |

## 3.8.1. Kết quả kiểm thử

| Module | File test | Số test | Nội dung kiểm tra | Kết quả |
|--------|----------|---------|-------------------|--------|
| ToursService | `tours.service.spec.ts` | 6 | Danh sách tour phân trang, chi tiết tour (NotFoundException), danh mục | ✅ Passed |
| ReviewsService | `reviews.service.spec.ts` | 8 | 4 validation rules + success case, pagination | ✅ Passed |
| TourRequestsService | `tour-requests.service.spec.ts` | 6 | Tour not found, self-booking, capacity, duplicate | ✅ Passed |
| **Tổng** | **3 suites** | **20 tests** | **0 failed, 0.909s** | ✅ **All Passed** |

Kỹ thuật: Jest mock cho PrismaService, kiểm tra NestJS exceptions (NotFoundException, ForbiddenException, BadRequestException, ConflictException), business rules validation.
...
4. **Phân quyền admin chi tiết** giữa CONTENT_MODERATOR và SUPPORT_STAFF chưa phân biệt ở frontend.
5. **Chat** chưa hỗ trợ gửi file/ảnh, chưa có typing indicator.
6. **Chưa triển khai production** — dự án chạy ở localhost.
7. **Kiểm thử tự động** đã có unit test cho 3 module lõi (20 test cases), nhưng chưa có E2E test.
