...
> Phiên bản đã điều chỉnh phần thanh toán từ **PayPal Sandbox** sang **VNPAY Sandbox** dựa trên tài liệu tích hợp VNPAY và định hướng nghiệp vụ đã phân tích cho đồ án.
...
Tài liệu Sprint 13 cần được điều chỉnh từ định hướng PayPal Sandbox sang **VNPAY Sandbox**.

Trong phạm vi đồ án sinh viên, hướng đi phù hợp là:

- ưu tiên **VNPAY Sandbox** để minh họa đúng bối cảnh thanh toán phổ biến tại Việt Nam;
- vẫn có thể giữ **mock flow nội bộ** như phương án dự phòng nếu việc tích hợp sandbox phát sinh lỗi kỹ thuật;
- chỉ cần làm rõ các bước chính:
  - tạo giao dịch nội bộ;
  - tạo URL thanh toán VNPAY;
  - chuyển hướng người dùng sang VNPAY;
  - nhận kết quả tại `vnp_ReturnUrl`;
  - nhận thông báo xác nhận tại `IPN URL`;
  - cập nhật trạng thái giao dịch và lịch sử thanh toán.

Theo tài liệu VNPAY PAY Sandbox, merchant phải tự cài đặt 3 phần gồm: build URL thanh toán chuyển hướng, xử lý `vnp_ReturnUrl`, và xử lý `IPN URL`; kết quả thanh toán nên được cập nhật ở IPN thay vì ReturnUrl. citeturn199315view0
...
Phải chốt state machine của `payment_transactions` theo hướng phù hợp với VNPAY Sandbox và đủ đơn giản cho đồ án.

Bộ trạng thái khuyến nghị:

- `pending`: giao dịch đã được tạo nội bộ nhưng chưa có kết quả cuối cùng;
- `processing`: hệ thống đang nhận hoặc xử lý kết quả từ VNPAY;
- `paid`: thanh toán thành công và đã được backend xác nhận qua IPN;
- `failed`: thanh toán thất bại theo phản hồi từ cổng thanh toán;
- `cancelled`: người dùng hủy hoặc giao dịch không tiếp tục;
- `expired`: quá hạn thanh toán;
- `refunded`: trạng thái dự phòng cho hướng mở rộng, không bắt buộc demo.

Nếu muốn giữ phiên bản rút gọn cho báo cáo và demo, có thể dùng 4 trạng thái lõi:
...
Điểm quan trọng là phải nhất quán giữa database, backend, frontend và mô tả nghiệp vụ. Với luồng VNPAY, nên hiểu rằng trạng thái thành công chỉ được chốt khi backend nhận và xác minh IPN hợp lệ từ VNPAY. Tài liệu VNPAY cũng yêu cầu merchant kiểm tra checksum, đối chiếu đơn hàng, đối chiếu số tiền và trạng thái đơn trước khi xác nhận giao dịch. citeturn199315view0

### 3.8. Có cần tích hợp payment thật hay chỉ mock

Phải chốt ngay từ đầu rằng Sprint 13 ưu tiên **VNPAY Sandbox** ở mức có thể demo được.

Hai phương án hợp lý là:

- dùng **VNPAY Sandbox thật** để thể hiện rõ luồng tạo URL thanh toán, ReturnUrl và IPN;
- giữ **mock flow nội bộ** như phương án fallback nếu thời gian hoặc môi trường tích hợp không ổn định.

Không nên để tới cuối sprint mới quyết định, vì lựa chọn này ảnh hưởng trực tiếp tới:

- thiết kế API payment;
- cấu trúc trạng thái `payment_transactions`;
- cách frontend xử lý điều hướng thanh toán;
- cách backend xử lý callback và lịch sử giao dịch.

Trong phiên bản chốt cho đồ án, nên mô tả payment theo hướng **ưu tiên VNPAY Sandbox, có thể fallback mock flow nếu cần bảo đảm tiến độ bảo vệ**.
...
- Cổng thanh toán sử dụng: **VNPAY Sandbox**.
- URL thanh toán Sandbox, `vnp_TmnCode`, `vnp_HashSecret`.
- Mã giao dịch nội bộ và `vnp_TxnRef`.
- State machine của `payment_transactions`.
- Mối liên hệ giữa `payment_transactions` và `tour_requests`.
- `vnp_ReturnUrl` để hiển thị kết quả cho người dùng.
- `IPN URL` để backend cập nhật trạng thái giao dịch.
- Màn hình lịch sử thanh toán và màn hình kết quả thanh toán.
- Cách xác nhận thanh toán thành công hoặc thất bại.

Theo tài liệu VNPAY, môi trường Sandbox sử dụng URL thanh toán `https://sandbox.vnpayment.vn/paymentv2/vpcpay.html`, cần `vnp_TmnCode`, `vnp_HashSecret`, và có thêm URL query/refund Sandbox `https://sandbox.vnpayment.vn/merchant_webapi/api/transaction`. citeturn199315view0
...
- **VNPAY Sandbox** là phương án chính thức;
- **mock flow nội bộ** là phương án dự phòng nếu cần giảm rủi ro demo.

Trong tài liệu sprint nên mô tả theo hướng:

- kiến trúc payment ưu tiên hỗ trợ VNPAY Sandbox;
- backend có khả năng tạo URL thanh toán, xử lý ReturnUrl và IPN;
- triển khai thực tế có thể fallback sang mock flow nếu mục tiêu là bảo vệ đồ án ổn định hơn.

Lý do chọn VNPAY Sandbox thay cho PayPal Sandbox là phù hợp hơn với bối cảnh website du lịch tại Việt Nam, đồng thời VNPAY cung cấp mô hình tích hợp khá rõ cho sandbox với 3 điểm xử lý chính: build URL thanh toán, `vnp_ReturnUrl`, và `IPN URL`. citeturn199315view0
...
- sinh URL thanh toán VNPAY cho người dùng;
- điều hướng sang cổng thanh toán VNPAY Sandbox;
- nhận kết quả hiển thị tại `vnp_ReturnUrl`;
- cập nhật kết quả thanh toán tại `IPN URL`;
- xem thông tin giao dịch và lịch sử thanh toán của user.
...
- đối soát production;
- đa cổng thanh toán;
- hóa đơn điện tử hoàn chỉnh;
- xử lý tài chính ở mức production.

Phạm vi này bám đúng tài liệu VNPAY ở chỗ merchant cần tự xử lý URL thanh toán, ReturnUrl và IPN; còn trong đồ án, phần cần làm sâu nhất là luồng cập nhật trạng thái giao dịch cho `payment_transactions` và `tour_requests`. citeturn199315view0
...
M22 chỉ cần mô phỏng rõ flow VNPAY Sandbox:

- chọn thanh toán cho `tour_request`;
- tạo giao dịch nội bộ;
- tạo URL thanh toán VNPAY;
- chuyển hướng người dùng sang VNPAY;
- quay về màn hình kết quả qua `vnp_ReturnUrl`;
- backend cập nhật trạng thái qua `IPN URL`;
- hiển thị lịch sử thanh toán.

Không nên làm:

- nhiều cổng thanh toán thật;
- xử lý đối soát phức tạp;
- hoàn tiền thật;
- nghiệp vụ kế toán;
- bảo mật production-grade vượt quá phạm vi đồ án.

Điểm cốt lõi là người xem phải hiểu được hệ thống có một luồng thanh toán hợp lý, tách rõ phần hiển thị kết quả và phần cập nhật dữ liệu. Tài liệu VNPAY nhấn mạnh rằng ReturnUrl dùng để hiển thị kết quả giao dịch, còn merchant cập nhật kết quả thanh toán qua IPN. citeturn199315view0
...
#### Mục tiêu
Mô phỏng bước thanh toán cho yêu cầu tham gia tour bằng **VNPAY Sandbox** theo đúng logic tích hợp của merchant.
...
#### Mức độ triển khai phù hợp
- VNPAY Sandbox hoặc mock fallback;
- không production;
- không làm sâu hoàn tiền hoặc đối soát.

Trong phiên bản triển khai, `tour_request` nên là đối tượng trung tâm của payment để luồng nghiệp vụ rõ ràng hơn và dễ đồng bộ với các sprint trước.
...
- thông tin `tour_request` liên quan;
- thông tin tour, số tiền, mô tả giao dịch;
- trạng thái thanh toán;
- nút tạo giao dịch thanh toán VNPAY;
- khu vực hiển thị kết quả trả về từ `vnp_ReturnUrl`;
- lịch sử thanh toán của user;
- trạng thái giao dịch mới nhất.

### Hành vi chính

- tạo `payment_transaction`;
- gọi backend để sinh URL thanh toán VNPAY;
- điều hướng người dùng sang VNPAY Sandbox;
- sau khi quay về hệ thống, hiển thị kết quả tại ReturnUrl;
- backend tiếp nhận IPN và cập nhật trạng thái cuối cùng;
- frontend tải lại trạng thái giao dịch để hiển thị.

### Điều cần tránh

- không mô phỏng quá nhiều cổng thanh toán;
- không làm luồng checkout quá dài;
- không làm refund thật;
- không cập nhật kết quả thanh toán trực tiếp ở frontend chỉ dựa trên ReturnUrl.

Cách mô tả này phù hợp hơn với VNPAY vì tài liệu tích hợp tách rõ 2 nhánh: `vnp_ReturnUrl` cho hiển thị kết quả và `IPN URL` cho cập nhật trạng thái thanh toán. citeturn199315view0
...
### Dùng cho
- tạo giao dịch nội bộ trước khi redirect sang VNPAY;
- lưu mã giao dịch nội bộ;
- lưu mã giao dịch từ cổng thanh toán nếu có;
- cập nhật trạng thái giao dịch sau IPN;
- xem lịch sử payment của user;
- hiển thị payment status ở M22 và các màn hình liên quan.
...
### Gợi ý vận hành
- `payment_method` nên chốt là `vnpay` cho Sprint 13;
- `transaction_code` hoặc một mapping từ transaction nội bộ phải liên kết được với `vnp_TxnRef`;
- dữ liệu `gateway_response` nên lưu thông tin phản hồi cần thiết từ VNPAY để tiện tra cứu khi demo.

Nhóm dữ liệu này phải được thiết kế sao cho hỗ trợ được cả bước tạo URL thanh toán lẫn bước cập nhật kết quả từ IPN.
...
- sinh `transaction_code` / `vnp_TxnRef`;
- build URL thanh toán VNPAY và trả về cho frontend.

### `GET /payments/vnpay/return`
Mục đích:

- nhận query parameters từ VNPAY khi người dùng quay về hệ thống;
- kiểm tra checksum;
- hiển thị kết quả cho người dùng;
- không cập nhật kết quả cuối cùng vào database.

### `GET /payments/vnpay/ipn`
Mục đích:

- nhận thông báo server-to-server từ VNPAY;
- kiểm tra checksum;
- tìm giao dịch nội bộ;
- đối chiếu số tiền và trạng thái;
- cập nhật `payment_transactions`;
- cập nhật `tour_requests`;
- trả `RspCode` và `Message` cho VNPAY.
...
### `GET /me/tour-requests/:tourRequestId/payment`
Mục đích:

- lấy giao dịch gần nhất hoặc giao dịch liên quan đến một yêu cầu tham gia tour.

### `POST /me/tour-requests/:tourRequestId/payments/vnpay/retry`
Mục đích:

- tạo lại giao dịch mới khi giao dịch cũ thất bại, bị hủy hoặc hết hạn;
- vẫn giữ đúng điều kiện `tour_request` còn ở trạng thái cho phép thanh toán.

Nhóm API này bám sát flow VNPAY hơn cách thiết kế generic `POST /payments/:id/confirm`, vì VNPAY thực tế dùng ReturnUrl và IPN thay cho mô hình “confirm nội bộ” đơn giản. citeturn199315view0
...
- dựng giao diện tóm tắt giao dịch thanh toán;
- hiển thị thông tin `tour_request` liên quan;
- nút tạo URL thanh toán VNPAY;
- xử lý điều hướng người dùng sang VNPAY Sandbox;
- xây màn hình hoặc route nhận kết quả từ `vnp_ReturnUrl`;
- hiển thị trạng thái `pending / paid / failed / cancelled`;
- hiển thị lịch sử thanh toán của user;
- xử lý polling hoặc reload trạng thái sau khi backend nhận IPN nếu cần.

Frontend phải thể hiện được rằng kết quả hiển thị cho người dùng và kết quả cập nhật trong database là hai bước liên quan nhưng không hoàn toàn trùng nhau.
...
- tạo `payment_transaction` mới cho một `tour_request`;
- sinh `transaction_code` và map với `vnp_TxnRef`;
- build bộ tham số VNPAY và ký checksum;
- trả về payment URL cho frontend;
- xử lý `vnp_ReturnUrl` để kiểm tra checksum và hiển thị kết quả;
- xử lý `IPN URL` để cập nhật trạng thái payment;
- cập nhật `tour_requests` khi thanh toán thành công;
- hỗ trợ lấy lịch sử thanh toán theo user;
- hỗ trợ retry transaction khi giao dịch cũ chưa thành công.

Module payments cần bám đúng flow merchant của VNPAY thay vì chỉ mô phỏng một API `confirm` nội bộ. Theo tài liệu VNPAY, merchant phải cài đặt code build URL thanh toán, code `vnp_ReturnUrl` và code `IPN URL`. citeturn199315view0
...
- có dữ liệu cho `paid / failed / pending / cancelled`;
- có một số mẫu phản hồi gateway để minh họa tra cứu;
- nếu có thể, chuẩn bị dữ liệu mô phỏng tương ứng với `vnp_TxnRef` và kết quả callback.

Dữ liệu demo của payment nên đủ để minh họa 2 trường hợp quan trọng nhất:

- thanh toán thành công và cập nhật `tour_requests = paid`;
- thanh toán không thành công nhưng `tour_requests` vẫn giữ trạng thái chờ thanh toán để thử lại.
...
## 14.1. Activity Diagram hoàn thiện

Dưới đây là các sơ đồ hoạt động (Activity Diagrams) mô tả luồng xử lý của các tính năng mới trong Sprint 13.

### Luồng 1: Chatbot AI tư vấn du lịch (F22)
```mermaid
activityDiagram
    start
    :User truy cập trang Trợ lý AI;
    :Hệ thống lấy danh sách phiên hội thoại (sessions);
    if (Đã có session?) then (Có)
        :User chọn session cũ;
    else (Không)
        :User nhấn "Bắt đầu hội thoại mới";
        :Hệ thống gọi API tạo session;
    endif
    :User nhập câu hỏi du lịch;
    :Hệ thống hiển thị tin nhắn tạm thời (optimistic update);
    :Backend nhận tin nhắn và gọi Gemini AI SDK;
    :Gemini AI xử lý dựa trên Context và System Instruction;
    :Backend lưu tin nhắn của AI vào Database;
    :Frontend nhận phản hồi và cập nhật UI;
    stop
```

### Luồng 2: Thanh toán trực tuyến VNPAY Sandbox (F24)
```mermaid
activityDiagram
    start
    :User xem danh sách yêu cầu Tour đã gửi;
    :User nhấn "Thanh toán" cho yêu cầu hợp lệ;
    :Hệ thống gọi API create-vnpay-url;
    :Backend tạo bản ghi Giao dịch (pending);
    :Backend tạo URL thanh toán VNPAY (vnp_Url);
    :Frontend chuyển hướng sang Cổng thanh toán VNPAY;
    :User nhập thông tin thẻ Test tại VNPAY;
    :VNPAY xử lý và trả kết quả về ReturnUrl;
    :Frontend VnpayReturnPage nhận các tham số vnp_ResponseCode;
    if (vnp_ResponseCode == '00'?) then (Thành công)
        :Hệ thống gọi API cập nhật trạng thái;
        :Backend cập nhật TourRequest = paid;
        :Backend cập nhật Transaction = paid;
        :Hiển thị thông báo thành công;
    else (Thất bại)
        :Hiển thị thông báo lỗi;
        :Backend cập nhật Transaction = failed;
    endif
    stop
```

### Luồng 3: Gợi ý Tour thông minh (F21)
```mermaid
activityDiagram
    start
    :User đăng nhập và truy cập Trang chủ;
    :Hệ thống gọi API getRecommendations;
    :Backend lấy User Preferences (vùng miền, loại tour);
    :Backend truy vấn danh sách Tour phù hợp tiêu chí;
    :Backend sắp xếp và tạo match_reasons;
    :Frontend nhận danh sách tour gợi ý;
    :Hiển thị tại Section "Gợi Ý Dành Riêng Cho Bạn";
    stop
```

### Luồng 4: Xem lưu trú liên quan (F23)
```mermaid
activityDiagram
    start
    :User xem Chi tiết Tour;
    :Hệ thống gọi API getTourAccommodations;
    :Backend truy vấn bảng tour_accommodations;
    :Backend kết hợp thông tin từ bảng partner_accommodations;
    :Frontend nhận danh sách điểm lưu trú;
    :Hiển thị tại Tab Tổng quan của Tour;
    stop
```
...
- sequence “User tạo URL thanh toán VNPAY và hệ thống nhận ReturnUrl/IPN”;
- sequence “User lấy recommendation”.

Trong đó, sequence cho payment nên mô tả tối thiểu các actor:
...
- có payment flow ở mức **VNPAY Sandbox hoặc mock fallback**.
...
- M22 mô phỏng được luồng thanh toán VNPAY.
...
- API payment theo flow VNPAY chạy được ở mức demo.
...
- phần payment được chuyển từ PayPal Sandbox sang VNPAY Sandbox;
...
- payment đi theo đúng logic VNPAY gồm create URL, ReturnUrl và IPN;
